...
class CoverageReport {
  static generate(coverageFile, aux4Dir, threshold) {
    ...
    const stepPct = CoverageReport.printReport(universe, hitMap, coverageData);

    if (threshold > 0 && stepPct < threshold) {
      console.error(`Coverage ${stepPct}% is below threshold ${threshold}%`);
      return false;
    }
  ...
  static printReport(universe, hitMap, coverageData) {
    ...
    return totalSteps > 0 ? Math.round((coveredSteps / totalSteps) * 100) : 0;
  }
